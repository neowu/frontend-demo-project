...
const typeSourcePath = path.resolve(__dirname, `../app/${typeModule}/api.ts`);
const serviceModule = "service/api";
const serviceSourceDir = path.resolve(__dirname, `../app/${serviceModule}`);
...
function generateService(serviceName, operations) {
    ...
    const lines = [];
    lines.push(`import {NetworkService} from "app/service/NetworkService";`);
    lines.push(``);
    lines.push(`// Attention: This file is generated by "yarn api", do not modify`);
    lines.push(``);
    ...
    operations.forEach(operation => {
        ...
        lines.push(`return NetworkService.ajax("${operation.method}", "${operation.path}", ${pathParams}, ${requestBody});`);
        lines.push("}");
        lines.push("");
    });

    lines.push(`}`);
    if (requiredTypes.length > 0) lines.unshift(`import {${requiredTypes.join(",")}} from "app/${typeModule}/api";`);
...
function formatSources() {
    ...
    spawn("prettier", ["--config", "node/prettier.json", "--write", `app/{${serviceModule},${typeModule}}/*.ts`]);
}
